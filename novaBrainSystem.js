...
function buildSubscribeInterestReply() {
  return `📬 يسعدني حماسك للمتابعة<br>
بدل التشتت بين عشرات المصادر،<br>
يمكنك أن تصلك الخلاصة مباشرة — بهدوء، وبدون إزعاج.`;
}

function buildBusinessSubscribeReply() {
  return `👨‍💻 كثير من روّاد الأعمال يشعرون أن الذكاء الاصطناعي “مهم”…<br>
لكنهم لا يجدون وقتًا لتجربة كل أداة أو متابعة كل تحديث.<br>
هنا نحاول اختصار الطريق، لا تعقيده.`;
}

function buildConsultingPurchaseReply() {
  return `💬 أغلب المشاريع لا تخسر بسبب ضعف المنتج،<br>
الحل أحيانًا أبسط مما نتوقع.`;
}

function buildCollaborationReply() {
  return `🤝 إن كنت تفكّر بتعاون، شراكة، أو فكرة مشتركة ذات قيمة حقيقية،<br>
