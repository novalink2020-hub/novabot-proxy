...
/**
 * فلترة وتحسين أفضل تطابق:
 * - منع سياسة الخصوصية / شروط الاستخدام إلا عند طلب صريح.
 * - تفضيل صفحة الخدمات عند سؤال عن خدمات نوفا لينك.
 * - تفضيل صفحة من نحن عند سؤال "من أنتم / من هي نوفا لينك".
 */
function postProcessBestMatch(userText = "", bestMatch, items) {
  let { score, item } = bestMatch || { score: 0, item: null };
  if (!items || !items.length) return { score, item };

  const q = (userText || "").toLowerCase();

  const isPolicy =
    item &&
    (item.url.includes("syash-alkhswsyh") ||
      item.url.includes("shrwt-alastkhdam"));

  const mentionsPolicy = /سياسة|خصوصية|privacy|شروط الاستخدام|terms/i.test(
    userText
  );

  // 1) منع ربط سياسة الخصوصية / الشروط إلا عند طلبها
  if (isPolicy && !mentionsPolicy) {
    item = null;
    score = 0;
  }

  // 2) تفضيل صفحة الخدمات عند سؤال عن خدمات نوفا لينك
  const wantsServices =
    /خدمات|خدمتكم|خدمتك|service/i.test(userText) &&
    /نوفا لينك|novalink|nova link/i.test(userText);

  if (wantsServices) {
    const servicesItem =
      items.find((x) =>
        (x.url || "").includes("services-khdmat-nwfa-lynk")
      ) ||
      items.find((x) =>
        (x.title || "").toLowerCase().includes("خدمات")
      );
  ...
  // 3) تفضيل صفحة من نحن عند أسئلة "من أنتم / من هي نوفا لينك / من نحن"
  const wantsAbout = /من انتم|من أنتم|من هي نوفا لينك|من انتي نوفا لينك|من نحن|about us/i.test(
    userText
  );
  ...
  if (wantsAbout) {
    const aboutItem =
      items.find((x) =>
        (x.url || "").includes("about-us-althkaa-alastnaay")
      ) ||
      items.find((x) =>
        (x.title || "").toLowerCase().includes("من نحن")
      );
...
// نمنع استدعاء Gemini لنوايا معينة + أي شيء خارج نطاق العمل والـ AI
...
export async function novaBrainSystem(request = {}) {
  ...
  if (kb.length) {
    ...
    bestMatch = postProcessBestMatch(userText, bestMatch, kb);
  }
