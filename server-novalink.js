...
function analyzeAIFlow(userMessage, analysis, history) {
  ...
  // كلمات مرتبطة بأدوات/منصات ذكاء اصطناعي حتى لو بدون كلمة "ذكاء"
  const aiToolHints = [
    "شات جي بي تي",
    "chatgpt",
    "chat gpt",
  ...
  // بدايات أسئلة بالعربية والإنجليزية
  ...
  // 1) أي ذكر واضح لأداة ذكاء اصطناعي → نميّل لاستخدام Gemini
  if (aiToolHints.some((kw) => text.includes(kw))) {
    return true;
  }

  // 2) جملة على شكل سؤال واضح
  if (trimmed.endsWith("?")) {
    return true;
  }
  if (
...
(async () => {
  if (!KNOWLEDGE_URL) {
    console.warn("⚠️ KNOWLEDGE_V5_URL / KNOWLEDGE_JSON_URL is not set. NovaBot will rely on Gemini + automated replies only.");
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  req.on("end", async () => {
    ...
    try {
      ...
      // 2) قرار هل نجبر واجهة الذكاء الاصطناعي أم لا
      const forceAI = analyzeAIFlow(userMessage, analysis, history);

      // لو أجبرنا AI → نغيّر النية إلى ai_business ولا نمرّر suggestedCard اشتراك/تعاون
